...
public class RabbitMQTransport implements MessageTransport {
     ...
     */
    private TransportResult sendTo(TransportEvent message) {
        try {
            // 获取路由上下文
            ...
            if (EventModel.BROADCAST.equals(message.getEventModel())) {
                routingKey = "";
            }
            ...
            log.debug("Message sent to {}: exchange={}, routingKey={}, queue={}, messageId={}",
                    message.getEventModel().name(), exchangeName, routingKey, queueName, message.getEventId());
